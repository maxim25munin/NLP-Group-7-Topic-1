...
- **Results.**
  ...
  - Contextual comparison: a single-epoch XLM-R fine-tune reaches **0.9950** ID accuracy but only **0.9431** macro OOD accuracy—a **0.0519** drop—while the fastText baseline stays at **0.9859** OOD and a **0.0116** drop, showing the pretrained embeddings remain stronger under this lightweight setup.[^xlmr-summary]
...
[^xlmr-summary]: fastText vs. XLM-R ID/OOD summary table from `reports/Evaluate XLM-RoBERTa versus fastText on non-English OOD language identification. run 2.1.2026.md` (lines 716–805).
