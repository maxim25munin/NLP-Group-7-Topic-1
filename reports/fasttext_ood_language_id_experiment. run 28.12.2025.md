...
This notebook follows the Question 1 guidance by evaluating pretrained fastText embeddings on five languages: Kazakh, Latvian, Swedish, Yoruba, and Urdu. Wikipedia-derived CoNLL-U data are used for Latvian, Swedish, Yoruba, and Urdu, while the OOD hate-speech/social-media corpora come from `data/kazakh_hate_speech_fasttext.csv`, `data/afrihate_yoruba_fasttext.csv`, `data/latvian_comments_fasttext_nat_only_20mb.csv`, `data/biaswe_fasttext.csv`, and `data/gsm8k_urdu_fasttext.csv`. The goal is to illustrate how relying on pretrained fastText embeddings for language identification can break when confronting non-Wikipedia, out-of-distribution content across multiple languages.
...
- OOD hate-speech/social-media CSV files at `data/kazakh_hate_speech_fasttext.csv`, `data/afrihate_yoruba_fasttext.csv`, `data/latvian_comments_fasttext_nat_only_20mb.csv`, `data/biaswe_fasttext.csv`, and `data/gsm8k_urdu_fasttext.csv` with columns `text` and `label`.
...
OOD_LANGUAGES = ["kazakh", "latvian", "swedish", "yoruba", "urdu"]
...
OOD_FILES: Dict[str, Path] = {
    ...
    "yoruba": DATA_DIR / "afrihate_yoruba_fasttext.csv",
    "latvian": DATA_DIR / "latvian_comments_fasttext_nat_only_20mb.csv",
    "swedish": DATA_DIR / "biaswe_fasttext.csv",
    "urdu": DATA_DIR / "gsm8k_urdu_fasttext.csv",
}
...
The next cell loads the Wikipedia in-distribution (ID) data for Kazakh, Latvian, Swedish, Yoruba, and Urdu, then performs a reproducible train/validation split. It also loads the Kazakh, Yoruba, Latvian, Swedish, and Urdu hate-speech/social-media OOD datasets, which are held out entirely for OOD evaluation.
...
```
    ...
    Loaded 110607 latvian OOD sentences (held out for OOD evaluation)
    Loaded 450 swedish OOD sentences (held out for OOD evaluation)
    Loaded 4856 yoruba OOD sentences (held out for OOD evaluation)
    Loaded 6365 urdu OOD sentences (held out for OOD evaluation)
...
```
    ...
    OOD accuracy (latvian hate speech/social media): 0.9840
    {
      "kazakh": {
        "precision": 0.0,
        "recall": 0.0,
      ...
      "accuracy": 0.9839883551673945,
      "macro avg": {
        "precision": 0.3333333333333333,
        "recall": 0.3279961183891315,
        "f1-score": 0.330643189043776,
        "support": 110607.0
      },
      "weighted avg": {
        "precision": 1.0,
    ...
    C:\Users\Maxim\conda\lib\site-packages\sklearn\metrics\_classification.py:407: UserWarning: A single label was found in 'y_true' and 'y_pred'. For the confusion matrix to have the correct shape, use the 'labels' parameter to pass all known labels.
      warnings.warn(
    

    OOD accuracy (swedish hate speech/social media): 1.0000
    {
      "swedish": {
        "precision": 1.0,
        "recall": 1.0,
    ...
    OOD accuracy (urdu hate speech/social media): 1.0000
    {
      "urdu": {
        "precision": 1.0,
        "recall": 1.0,
    ...
    OOD accuracy (yoruba hate speech/social media): 0.9996
    {
      "kazakh": {
        "precision": 0.0,
        "recall": 0.0,
      ...
      "yoruba": {
        "precision": 1.0,
        "recall": 0.9995881383855024,
        "f1-score": 0.9997940267765191,
        "support": 4856.0
      },
      "accuracy": 0.9995881383855024,
      "macro avg": {
        "precision": 0.5,
        "recall": 0.4997940691927512,
        "f1-score": 0.49989701338825954,
        "support": 4856.0
      },
      "weighted avg": {
        "precision": 1.0,
    ...
    Macro OOD accuracy across languages: 0.9863
    {
      "kazakh": {
        "precision": 0.9997032934427851,
        "recall": 0.9958620689655172,
        "f1-score": 0.9977789842554662,
        "support": 10150.0
      },
      "latvian": {
        "precision": 1.0,
      ...
      "yoruba": {
        "precision": 0.7281728172817282,
        "recall": 0.9995881383855024,
        "f1-score": 0.8425620551987503,
        "support": 4856.0
      },
      "accuracy": 0.9862944392424563,
      "macro avg": {
        "precision": 0.9455752221449026,
        "recall": 0.9958877125036828,
        "f1-score": 0.966454121317109,
        "support": 132428.0
      },
      "weighted avg": {
        "precision": 0.9900096326242511,
        "recall": 0.9862944392424563,
        "f1-score": 0.9873160635359055,
        "support": 132428.0
      }
    }
...
```
    ...
    0   kazakh       0.144956      0.084897       0.070526             30299
    1  latvian       0.159612      0.240599       0.016613            469222
    2  swedish       0.128034      0.122019       0.106610             12394
    3     urdu       0.091078      0.131445       0.253614              8570
    4   yoruba       0.298034      0.379225       0.085711             20801
