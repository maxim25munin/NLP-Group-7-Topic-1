...
def extract_fasttext_features(
    ...
    If `language_hint` is provided and exists in the model lookup, that model is
    used for all texts (useful for OOD Kazakh-only evaluation). Otherwise the
    function attempts to match each sample's language label to a loaded model;
    when no match is found, it falls back to the first available model.
    """
    default_model = models[language_hint] if language_hint and language_hint in models else models[sorted(models.keys())[0]]
    features: List[np.ndarray] = []
    for i, text in enumerate(texts):
        model = default_model
        if language_labels and i < len(language_labels):
            lang = language_labels[i]
            model = models.get(lang, default_model)
...
hate_train_df, hate_ood_df = train_test_split(
    hate_df, test_size=0.2, random_state=RANDOM_SEED, stratify=hate_df.label
)
print(f"Kazakh hate-speech split — Train: {len(hate_train_df)}, OOD eval: {len(hate_ood_df)}")
...
combined_df = pd.concat([wiki_df, hate_train_df], ignore_index=True)

train_df, test_df = train_test_split(
    combined_df, test_size=0.2, random_state=RANDOM_SEED, stratify=combined_df.label
)
...
```
    ...
    Kazakh hate-speech split — Train: 8120, OOD eval: 2030
    Loaded 8000 Wikipedia sentences across 4 languages
    Train size: 12896, Test size: 3224
...
```
    ...
    In-distribution accuracy: 0.9988
    {
      "kazakh": {
        "precision": 1.0,
        "recall": 0.9975369458128078,
        "f1-score": 0.998766954377312,
        "support": 1624.0
      },
      "latvian": {
        "precision": 1.0,
      ...
      "macro avg": {
        ...
        "recall": 0.9995073891625615,
        "f1-score": 0.9987583659998405,
        "support": 3224.0
      },
      "weighted avg": {
        "precision": 0.9987715893177407,
        "recall": 0.9987593052109182,
        "f1-score": 0.9987616265997241,
        "support": 3224.0
      }
    }
...
```
    ...
    OOD accuracy (Kazakh hate speech): 0.9995
    {
      "kazakh": {
        "precision": 1.0,
        "recall": 0.9995073891625615,
        "f1-score": 0.9997536338999754,
        "support": 2030.0
      },
      "yoruba": {
        "precision": 0.0,
      ...
      "macro avg": {
        "precision": 0.5,
        "recall": 0.49975369458128077,
        "f1-score": 0.4998768169499877,
        "support": 2030.0
      },
      "weighted avg": {
        "precision": 1.0,
...
```
    ...
    OOV rate on Wikipedia test split: 42.40%
    OOV rate on hate-speech corpus: 9.09%
    Vocabulary overlap (hate-speech vs. Wikipedia): 58.25%
    Hate-speech-specific vocabulary items: 4633
    Sample: ['aamaq', 'abazubayr', 'abc', 'abramsтың', 'acab', 'afr', 'agb', 'alfurqan', 'aljazeera', 'alleyeson', 'amaq', 'amf', 'amphetamines', 'ancyl', 'aqmi', 'assad', 'assembly', 'aviationweek', 'avqeddet', 'aw']
